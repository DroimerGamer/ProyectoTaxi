...
import { TrendingDown, Plus, Pencil, Loader2, Download, Filter } from 'lucide-react'
...
export default function Gastos() {
  ...
  const [form, setForm] = useState({
    unidad_id: '', categoria_id: '', fecha: '', concepto: '', monto: '', notas: '',
  })
  ...
  return (
    <div>
              ...
              <tbody className="divide-y divide-pizarra-700/30">
                {gastos.map((g) => (
                  <tr key={g.id} className="hover:bg-pizarra-700/20 transition-colors">
                    ...
                    <td className="table-cell font-mono font-semibold text-taxi-400">#{g.unidades?.numero}</td>
                    <td className="table-cell text-pizarra-400">{g.categorias_gasto?.nombre || '—'}</td>
      ...
      {/* Modal */}
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editId ? 'Editar gasto' : 'Nuevo gasto'}>
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="label">Unidad *</label>
              <select className="input" value={form.unidad_id} onChange={(e) => setForm({ ...form, unidad_id: e.target.value })}>
